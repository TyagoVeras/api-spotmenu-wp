...
/* verificar condição para envio de dados no webhook (novas mensagens) */
exports.verifySend = async (message) => {

  return new Promise( async (resolve, reject) => {

      var result = false;

      if(message.fromMe == false  && message.isGroupMsg == false && message.from !== "status@broadcast"){
        result = true;       
      }

      /* mensagem enviada */
      if (confApi.send_post_php.sendForMe == true && message.fromMe == true){
            result = true;
      }

      /* mensagem grupo */
      if (confApi.send_post_php.sendForGroups == true && message.isGroupMsg == true){
            result = true;
      }     

       /* atualização de status */
      if (confApi.send_post_php.sendForStatus == true && message.from == "status@broadcast"){
           result = true;
      }
