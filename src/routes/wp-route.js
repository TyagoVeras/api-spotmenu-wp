...
var wpChatButtons = require('../controllers/ChatButtons_controller');
  ...
  */
  ...
  consulta = await verify_instance(instancia);
      ...
      if(consulta.flag_exist == false){
        ...
        var setIntance = {};
        cBrowser.configs.session = instancia;
        cBrowser.configs.puppeteerOptions.userDataDir = 'tokens/' + instancia;       
              ...
              for(var i =0;i < instancias.length; i++){
                ...
                if(instancias[i].name == session_name){                     
                  ...
                  if (statusSession == 'isLogged' || statusSession == 'inChat') {
                    console.log("Status "+ statusSession +" setando para syncronized.")
                    instancias[i].qrcode = "syncronized";
                    instancias[i].status = true;

                  } else if (statusSession == 'qrReadSuccess') {
                    console.log("Qrcode ligo alterando status de "+ statusSession +" para syncronized.")
                    instancias[i].qrcode = "syncronized";
                    instancias[i].status = true;
                    ...
                    if(instancias[i].name == session_name && !instancias[i].instancia){
                      ...
                      console.log("👉🏻 Dados da Sessão: " + instancias[i]);
                      instancias[i].qrcode = statusSession;
                      instancias[i].status = "DISCONETED";
                    }
        ...
        // console.log(client.page);
         ...
         //  console.log(client.onMessage());
          client.onAnyMessage( async message => {
            ...
            /* verificar condição para envio de mensagem para webhook */
            var send = await utils.verifySend(message);
            console.log("Condicional para envio para webhook: ", send)
            
            if (send == true){  /* não enviada pela sessão ativa */
...
exports.getAllChatListTransm = async function(req,res){
...
exports.sendMensagem = async function(req, res){
  ...
  var consulta;
   ...
   if(consulta.flag_exist == true && consulta.instancia !== undefined){
      ...
      console.log("Mensagem a ser enviada: ", requisicao);
      ...
      try{
        ...
        retorno = await inst.sendText(requisicao.number, requisicao.msg);

      }catch(error){

          res.status(200).send({'retorno':retorno, 'info':'Erro ao tentar enviar mensagem: ' + error.toString()});
          return;
...
/* enviar buttons de opção */
exports.sendListButtons = async (req, res) => {
...
//console.log(consulta);
    if(consulta.flag_exist == true){
      ...
      if(inst){      
          ...
          retorno = await wpChatButtons.sendListButtons(inst,requisicao).then((result) => {

                return result;

            }).catch((erro) => {
                //console.error('Error when sending: ', erro); //return object error
                console.log("👉Erro ao realizar o envio dos buttons!", erro);
                return false;
            });



        res.status(200).send({'instancia':requisicao.instancia,'retorno':retorno,'status':status}); 
...
exports.sendButtons = async (req, res) => {
  ...
  var consulta = await verify_instance(requisicao.instancia);
    ...
    if(!requisicao.msg || requisicao.msg == undefined){
      res.status(200).send({'instancia':requisicao.instancia,'status':status, 'error':'Digite uma mensagem.'});
      return;
    }

    if(!requisicao.buttons || requisicao.buttons == undefined){
      res.status(200).send({'instancia':requisicao.instancia,'status':status, 'error':'Informe o object buttons para a mensagem.'});
      return;
    }
    ...
    if(consulta.flag_exist == true){
      ...
      if(inst){      
          ...
          try{
            ...
            console.log("\r\nDados envio buttons: ", requisicao);
            retorno = await inst.sendText(requisicao.number, requisicao.msg,requisicao.buttons);
...
exports.checkNumberStatus = async function(req,res){
         ...
         for(var i =0;i < instancias.length; i++){
              ...
              if(instancias[i].name == requisicao.instancia){
                  ...
                  try {
                    ...
                    retorno = await inst.checkNumberStatus(requisicao.number);
                    if(retorno == 404){
                      retorno = false
                    }
...
exports.logoff = async function(req,res){
  ...
  if(!requisicao.instancia || requisicao.instancia == undefined || requisicao.instancia == ""){
      ...
      for(var i =0;i < instancias.length; i++){
        ...
        if(instancias[i].name == requisicao.instancia && !instancias[i].instancia){
          instancias.splice(i, 1);          
         // instancias[i].qrcode = "";
         ...
         for(var i =0;i < instancias.length; i++){           
              ...
              if(instancias[i].name == requisicao.instancia){               
                ...
                try {
                  ...
                  if(requisicao.remover_cache == true){
  //console.log("Conteudo da sessão conectada: ",inst);
                    await inst.logout();
